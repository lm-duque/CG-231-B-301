...
// ** FUNCION ORTOGONAL **
function ortogonal(obj) {
    var l = 2; // Izquierda (left)
    var d = 2; // Abajo (down)
    var n = 1; // Cerca (near)
    var r = 2; // Derecha (right)
    var u = 2; // Arriba (up)
    var f = 10; // Lejos (far)
    var matrizO = new THREE.Matrix4(); // Definir matriz traslacion
    matrizO.set(lado / (r - l), 0, 0, -((r + l) / (r - l)),
                   0, lado / (u - d), 0, -((u + d) / (u - d)),
                   0, 0, lado / (f - n), -((f + n) / (f - n)),
                   0, 0, 0, 1);
    obj.applyMatrix(matrizO);
}
ortogonal(cube);
